...
def extract_acceptance_criteria(ticket_text: str) -> list[str]:
    ...
    if not found_section:
        ...
        criteria = [line.strip() for line in lines if AC_LINE_RE.match(line)]
...
def mechanical_complexity_check(
    ...
    return (
        ...
        f"{n} acceptance criteria; scope conjunction(s) detected "
        f"({', '.join(repr(hit) for hit in conjunction_hits[:3])}). "
...
def build_split_prompt(
    ticket_content: str, mechanical_explanation: str, review_context: str = ""
) -> str:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Assess a Linear ticket for complexity and propose a child-ticket split if needed."
        ),
    ...
    if args.ticket_file_in is not None:
        ...
        render.print_line(
            f"-- Reviewing local file {args.ticket_file_in} instead of fetching "
            f"{args.ticket_id} from Linear."
        )
